...
public class YjkhAjmxServiceImpl implements YjkhAjmxService {
    ...
    public List<Map<String, Object>> list(Map<String, Object> map) {
    	...
    	if("4".contains(lb) && "a3".equals(map.get("colIndex"))) {
			...
			item.put("DFSM", "个人承办结案数（折算后）：" + String.format("%.2f", ysf) + "分；全院入额法官人均承办结案数（按15%折算后；2017年按7.5%折算后）：" + String.format("%.2f", zsf) + "分。"
    				+ "       【得分说明：达到全院入额法官人均承办案数的50%，达到计40分。每高于10%，加5分，不设上限。低于的且分值在本部门入额法官年人均分值的100%-20%之间的，每低于人均值20%扣5分；"
    				+ "分值在本部门入额法官年人均分值的20%以下的，每低于人均分值20%，扣10分。】");
			list.add(item);
		}
    	return list;
