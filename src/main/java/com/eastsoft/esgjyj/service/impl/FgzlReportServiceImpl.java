...
@Service("fgzlReportService")
public class FgzlReportServiceImpl implements FgzlReportService{
	@Autowired
	BaseDao baseDao;
	@Autowired
	...
	@Override
	public List<Map<String, Object>> fgzlList(Map<String, Object> map) {
	...
	public Map<String, Double> getJlajMap(String khid, String ksrq, String jzrq) {
		...
		String sql = "select FGZLBS from CASES where COURT_NO = '0F' " + SftjUtil.generateBaseWhere("")
		+ SftjUtil.generateYjWhere(ksrq, jzrq, "")
		+ "and FGZLBS IN " + sjybss;
		List<Map<String, Object>> list = baseDao.queryForList(sql);
		String sjybs = "", officeid = "";
		double count = 0;
		for(Map<String, Object> item : list) {
			sjybs = (String)item.get("FGZLBS");
	...
	public Map<String, Object> getCount(String khid) {
		String sql = "select OFFICEID from YJKH_KHDX where KHID = '" + khid + "' and DXTYPE IN ('4', '6')";
		List<Map<String, Object>> list = baseDao.queryForList(sql);
	...
	private List<String> getSjyList(String khid) {
		String sql = "select USERID from YJKH_KHDX where KHID = '" + khid + "' and DXTYPE IN ('4', '6')";
		List<Map<String, Object>> list = baseDao.queryForList(sql);
		String sjybs = "";
